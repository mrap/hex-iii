...
`IIIConnectionState` is the public enum mirroring the wire-level connection lifecycle. It is
exported from the `iii_sdk::types` submodule (`use iii_sdk::types::IIIConnectionState;`), not the
crate root.
...
The SDK re-exports the engine's structured introspection types from the `iii_sdk::types`
submodule (e.g. `use iii_sdk::types::FunctionInfo;`):
