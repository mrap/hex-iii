...
middleware; rarely needed by callers. Imported from the `iii.types` submodule
(`from iii.types import MessageType`), not the package root.
...
"reconnecting" | "failed"`. Imported from the `iii.types` submodule
(`from iii.types import IIIConnectionState`), not the package root; treat the connection as
established once `register_worker` returns.
