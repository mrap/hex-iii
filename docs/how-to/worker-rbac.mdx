...
Enable role-based access control for workers connecting to your iii instance, allowing you to authenticate connections, control function access, and add middleware.

## Context

Use this guide when you need to expose iii functions to external workers (browsers, mobile apps, third-party services) with authentication and authorization. For type definitions and configuration reference, see [iii-worker-manager](/workers/iii-worker-manager).
...
Your iii instance now has:
- A dedicated RBAC port (49135) for external workers
- Authentication via your auth function
- Fine-grained access control per function
- Optional middleware for validation and audit logging

<Info title="Type Reference">
  For complete type definitions (`AuthInput`, `AuthResult`, `MiddlewareFunctionInput`, etc.), see the [iii-worker-manager reference](/workers/iii-worker-manager#rbac-authentication).
</Info>
