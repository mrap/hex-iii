...
<Update label="0.14.0" description="May 2026">
  ...
  **Breaking.** The Rust SDK's function registration is collapsed into a single entry point that mirrors Node and Python:
  ...
  See [the migration entry](/changelog/0-14-0/align-rust-register-function-with-signature) for the full before/after diff, builder methods, and step-by-step migration.
  ...
  **Breaking.** Six items moved off the `III` / `ISdk` instance (or off the crate / package root) into a dedicated `helpers` submodule across the Rust, Node, Python, and Browser SDKs. The free functions take the iii client as their first argument. Imports now go through a subpath.

  Relocated:

  | Symbol | Rust | Node | Browser | Python |
  |---|---|---|---|---|
  | `create_channel` | `iii_sdk::helpers::create_channel` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |
  | `create_stream` | `iii_sdk::helpers::create_stream` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |
  | `extract_channel_refs` | `iii_sdk::helpers::extract_channel_refs` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |
  | `is_channel_ref` | `iii_sdk::helpers::is_channel_ref` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |
  | `ChannelDirection` | `iii_sdk::helpers::ChannelDirection` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |
  | `ChannelItem` | `iii_sdk::helpers::ChannelItem` | `iii-sdk/helpers` | `iii-browser-sdk/helpers` | `iii.helpers` |

  `register_trigger_type` and `unregister_trigger_type` stay on the instance — call them as `iii.register_trigger_type(...)` / `iii.unregister_trigger_type(...)` exactly as before.
